...
int main(int argc, char *argv[])
{
  ...
  bool write_probability_maps = false;
  ...
  try
  {
    ...
    ("prob", po::bool_switch(&write_probability_maps)->default_value(false), "enable output of probability maps")
    ;
  ...
  std::cout << "----------------------------------------" << std::endl;
  std::vector<std::shared_ptr<Node<Classifier>>> trees;
  for (int tree = 0; tree < count_trees; tree++)
  {
    std::shared_ptr<Node<Classifier>> root = std::make_shared<Node<Classifier>>();
  ...
  for (int img_index = 0; img_index < count_images; img_index++)
  {
    ...
    for (int z = 0; z < image_resampled.sizeZ(); z++)
    {
      for (int y = 0; y < image_resampled.sizeY(); y++)
      {
        for (int x = 0; x < image_resampled.sizeX(); x++)
        {
    ...
    // testing output
    std::vector<Image> probmaps(count_labels);
    for (auto& map : probmaps)
    {
      map = image_resampled.clone();
      map.dataType(ImageDataType::FLOAT);
      zeros(map);
    }
    Image labelmap = image_resampled.clone();
    labelmap.dataType(ImageDataType::BYTE);
    zeros(labelmap);

    // testing trees
    auto start_testing = std::chrono::high_resolution_clock::now();

    //for (auto& sample : testingSamples)
    tbb::parallel_for(0, static_cast<int>(testingData.size()), [&](int s)
    {
      const auto sample = testingData[s];
      ...
      for (const auto& tree : trees)
      {
        ...
        for (int c = 0; c < count_labels; c++)
        {
          probmaps[c](x, y, z) += static_cast<float>(node.predictor.distribution[c] / static_cast<double>(count_trees));
        }
      }

      float best_label = 0;
      float best_prob = probmaps[0](x, y, z);
      for (int c = 1; c < count_labels; c++)
      {
        auto prob = probmaps[c](x, y, z);
        if (prob > best_prob)
        {
          best_prob = prob;
          best_label = c;
      ...
      labelmap(x, y, z) = original_labels[best_label];
    });
    ...
    if (write_probability_maps)
    {
      for (int c = 0; c < count_labels; c++)
      {
        std::stringstream filename;
        filename << output_path << "/" << basename << "_probmap_" << original_labels[c] << ".nii.gz";
        itkio::save(probmaps[c], filename.str());
      }

      std::stringstream filename;
      filename << output_path << "/" << basename << "_lmap.nii.gz";
      itkio::save(labelmap, filename.str());
    }
    Image lab = image.clone();
    resample(labelmap, lab, Interpolation::NEAREST);
    lab.dataType(ImageDataType::BYTE);
    std::stringstream filename;
    filename << output_path << "/" << basename << "_labelmap.nii.gz";
    itkio::save(lab, filename.str());
  }
}
