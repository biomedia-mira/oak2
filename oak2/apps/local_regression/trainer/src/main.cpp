...
int main(int argc, char *argv[])
{
  ...
  std::vector<std::string> slabelmap;
  std::vector<std::string> slabelmaplists;
  ...
  try
  {
    ...
    ("labelmap", po::value<std::vector<std::string>>(&slabelmap)->multitoken(), "filename(s) of label maps of single training image")
    ...
    ("labelmaps", po::value<std::vector<std::string>>(&slabelmaplists)->multitoken(), "text file(s) listing label maps")
    ...
    ("predict_patches", po::bool_switch(&predict_patches)->default_value(false), "enable 3x3x3 patch prediction")
    ;
  ...
  strings_to_values(slabelmap, labelmap);

  std::vector<std::string> labelmaplists;
  strings_to_values(slabelmaplists, labelmaplists);
  ...
  int labelmap_channels = std::max(labelmaplists.size(), labelmap.size());
  std::vector<std::vector<std::string>> labelmap_filenames(labelmap_channels);
  for (int c = 0; c < labelmap_channels; c++)
  {
    if (labelmaplists.size() > 0)
    {
      std::string filename;
      std::vector<std::string> filenames;
      std::ifstream ifs(labelmaplists[c]);
      while (getline(ifs, filename))
      {
        boost::trim(filename);
        if (filename != "")
      ...
      labelmap_filenames[c] = filenames;
    }
    if (labelmap.size() == labelmap_channels) labelmap_filenames[c].push_back(labelmap[c]);
  }
  ...
  std::vector<std::vector<Image>> labelmaps;
  ...
  for (int i = 0; i < image_filenames[0].size(); i++)
  {
    ...
    // load label map
    std::vector<Image> labelmaps_resampled(labelmap_channels);
    for (int c = 0; c < labelmap_channels; c++)
    {
      Image lmap = itkio::load(labelmap_filenames[c][i]);
      if (reorient_images) lmap = reorient(lmap);
      Image labelmap_resampled = image_resampled.clone();
      resample(lmap, labelmap_resampled, Interpolation::LINEAR);
      labelmaps_resampled[c] = labelmap_resampled;
    }
    ...
    labelmaps.push_back(labelmaps_resampled);
  ...
  if (write_temp)
  {
    ...
    for (int img_index = 0; img_index < intensity_images.size(); img_index++)
    {
      ...
      for (int channel = 0; channel < labelmap_channels; channel++)
      {
        std::stringstream filename_lab;
        filename_lab << output_path << "/temp/img_" << img_index << "_lab_ch_" << channel << ".nii.gz";
        itkio::save(labelmaps[img_index][channel], filename_lab.str());
      }
  ...
  int label_dim = predict_patches ? 27 * labelmap_channels : labelmap_channels;

  auto start_forest = std::chrono::high_resolution_clock::now();
  for (int tree = 0; tree < remaining_trees; tree++)
  {
    // sampling training data
    std::vector<std::shared_ptr<DataSample<Eigen::VectorXd>>> trainingData;
    for (int img_index = 0; img_index < intensity_images.size(); img_index++)
    {
      ...
      // set sampling mask boundary to zero
      Image boundary_mask = img_resampled.clone();
      zeros(boundary_mask);
      Image boundary_mask_sub = subimage(boundary_mask, 1, 1, 1, boundary_mask.sizeX() - 2, boundary_mask.sizeY() - 2, boundary_mask.sizeZ() - 2);
      ones(boundary_mask_sub);
      mul(sampling_mask, boundary_mask, sampling_mask);
      ...
      for (int z = 0; z < img_resampled.sizeZ(); z++)
      {
        for (int y = 0; y < img_resampled.sizeY(); y++)
        {
          for (int x = 0; x < img_resampled.sizeX(); x++)
          {
            ...
            {
              Eigen::VectorXd label(label_dim);
              if (label_dim == labelmap_channels)
              {
                for (int c = 0; c < labelmap_channels; c++)
                {
                  label[c] = labelmaps[img_index][c](x, y, z);
                }
              }
              else
              {
                ...
                for (int c = 0; c < labelmap_channels; c++)
                {
                  for (int dz = -1; dz <= 1; dz++)
                  {
                    for (int dy = -1; dy <= 1; dy++)
                    {
                      ...
                      {
                        label[running_index++] = labelmaps[img_index][c](x + dx, y + dy, z + dz);
                      }
                    }
                  }
                }
              ...
              std::shared_ptr<DataSample<Eigen::VectorXd>> sample = std::make_shared<DataSample<Eigen::VectorXd>>();
    ...
    std::vector<DataSample<Eigen::VectorXd>*> trainingSamples;
    ...
    train_tree(root, trainingSamples, feature_factory, feature_set, label_dim, max_depth, min_samples, improve_parent);
