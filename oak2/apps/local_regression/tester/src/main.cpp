...
int main(int argc, char *argv[])
{
  ...
  try
  {
    ...
    ("output_channels", po::value<int>(&output_channels)->default_value(1), "number of output channels (i.e. regression dimensions)")
    ;
  ...
  for (int img_index = 0; img_index < count_images; img_index++)
  {
    ...
    for (int c = 1; c < channels; c++)
    {
      ...
      threshold(img, msk, lower, upper);
      Image msk_resampled = intensity_img[0].clone();
      resample(msk, msk_resampled, Interpolation::NEAREST);
      mul(mask_resampled, msk_resampled, mask_resampled);
    ...
    std::vector<DataSample<int>> testingData;
    for (int z = 1; z < image_resampled.sizeZ()-1; z++)
    {
      for (int y = 1; y < image_resampled.sizeY()-1; y++)
      {
        for (int x = 1; x < image_resampled.sizeX()-1; x++)
        {
          if (mask_resampled(x,y,z) != 0.0f)
          {
            DataSample<int> sample;
            sample.image_index = 0;
            sample.point = Eigen::Vector3d(x,y,z);
            testingData.push_back(sample);
          }
        }
      }
    }
    ...
    // testing output
    std::vector<Image> predmaps_mean(output_channels);
    std::vector<Image> predmaps_std(output_channels);
    for (size_t c = 0; c < output_channels; c++)
    {
      predmaps_mean[c] = image_resampled.clone();
      predmaps_mean[c].dataType(ImageDataType::FLOAT);
      zeros(predmaps_mean[c]);

      predmaps_std[c] = image_resampled.clone();
      predmaps_std[c].dataType(ImageDataType::FLOAT);
      zeros(predmaps_std[c]);
    }

    Image predcounts = image_resampled.clone();
    predcounts.dataType(ImageDataType::FLOAT);
    zeros(predcounts);

    // testing trees
    auto start_testing = std::chrono::high_resolution_clock::now();

    for (const auto& sample : testingData)
    {
      int x = static_cast<int>(sample.point[0]);
      int y = static_cast<int>(sample.point[1]);
      int z = static_cast<int>(sample.point[2]);

      for (const auto& tree : trees)
      {
        auto node = descend_tree(*tree, sample, feature_factory, feature_set, max_depth);
        Eigen::VectorXd label_mean = node.predictor.means;
        Eigen::VectorXd label_var = node.predictor.variances;
        if (label_mean.size() == output_channels)
        {
          for (size_t c = 0; c < output_channels; c++)
          {
            predmaps_mean[c](x, y, z) += label_mean[c];
            predmaps_std[c](x, y, z) += sqrt(label_var[c]);
          }
          predcounts(x, y, z)++;
        }
        else
          ...
          for (size_t c = 0; c < output_channels; c++)
          {
            for (int dz = -1; dz <= 1; dz++)
            {
              for (int dy = -1; dy <= 1; dy++)
              {
                ...
                {
                  predmaps_mean[c](x + dx, y + dy, z + dz) += label_mean[running_index++];
                  predmaps_std[c](x + dx, y + dy, z + dz) += sqrt(label_var[running_index++]);
                  if (c == 0) predcounts(x + dx, y + dy, z + dz)++;
                }
              }
            }
          }
    ...
    for (size_t c = 0; c < output_channels; c++)
    {
      for (int z = 0; z < predcounts.sizeZ(); z++)
      {
        for (int y = 0; y < predcounts.sizeY(); y++)
        {
          for (int x = 0; x < predcounts.sizeX(); x++)
          {
            if (predcounts(x, y, z) > 0)
            {
              predmaps_mean[c](x, y, z) /= predcounts(x, y, z);
              predmaps_std[c](x, y, z) /= predcounts(x, y, z);
            }
          }
        }
      }
    ...
    for (size_t c = 0; c < output_channels; c++)
    {
      std::stringstream filename_mean;
      filename_mean << output_path << "/" << basename << "_predmap_" << c << "_mean.nii.gz";
      itkio::save(predmaps_mean[c], filename_mean.str());

      std::stringstream filename_std;
      filename_std << output_path << "/" << basename << "_predmap_" << c << "_std.nii.gz";
      itkio::save(predmaps_std[c], filename_std.str());
    }
  }
}
