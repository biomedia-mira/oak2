...
int main(int argc, char *argv[])
{
  ...
  int count_labels = trees.size() > 0 ? trees[0]->predictor.distribution.size() : 0;
  ...
  for (int img_index = 0; img_index < count_images; img_index++)
  {
    ...
    std::vector<double> distribution(count_labels);
    ...
    for (const auto& tree : trees)
    {
      ...
      {
        distribution[c] += static_cast<float>(node.predictor.distribution[c] / static_cast<double>(count_trees));
      }
